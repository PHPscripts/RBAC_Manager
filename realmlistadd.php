...
$realmlist_add = NULL; // Initialize page object first

class crealmlist_add extends crealmlist {
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "realmlistview.php")
	...
	function LoadDefaultValues() {
		...
		$this->address->CurrentValue = "127.0.0.1";
		$this->localAddress->CurrentValue = "127.0.0.1";
		$this->localSubnetMask->CurrentValue = "255.255.255.0";
		$this->port->CurrentValue = 8085;
		$this->icon->CurrentValue = 0;
		$this->timezone->CurrentValue = 0;
		$this->allowedSecurityLevel->CurrentValue = 0;
		$this->gamebuild->CurrentValue = 12340;
	}
	...
	function AddRow($rsold = NULL) {
		...
		// address
		$this->address->SetDbValueDef($rsnew, $this->address->CurrentValue, "", strval($this->address->CurrentValue) == "");

		// localAddress
		$this->localAddress->SetDbValueDef($rsnew, $this->localAddress->CurrentValue, "", strval($this->localAddress->CurrentValue) == "");

		// localSubnetMask
		$this->localSubnetMask->SetDbValueDef($rsnew, $this->localSubnetMask->CurrentValue, "", strval($this->localSubnetMask->CurrentValue) == "");

		// port
		$this->port->SetDbValueDef($rsnew, $this->port->CurrentValue, 0, strval($this->port->CurrentValue) == "");

		// icon
		$this->icon->SetDbValueDef($rsnew, $this->icon->CurrentValue, 0, strval($this->icon->CurrentValue) == "");

		// timezone
		$this->timezone->SetDbValueDef($rsnew, $this->timezone->CurrentValue, 0, strval($this->timezone->CurrentValue) == "");

		// allowedSecurityLevel
		$this->allowedSecurityLevel->SetDbValueDef($rsnew, $this->allowedSecurityLevel->CurrentValue, 0, strval($this->allowedSecurityLevel->CurrentValue) == "");

		// gamebuild
		$this->gamebuild->SetDbValueDef($rsnew, $this->gamebuild->CurrentValue, 0, strval($this->gamebuild->CurrentValue) == "");
...
// Create page object
if (!isset($realmlist_add)) $realmlist_add = new crealmlist_add();

// Page init
$realmlist_add->Page_Init();

// Page main
$realmlist_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var realmlist_add = new ew_Page("realmlist_add");
realmlist_add.PageID = "add"; // Page ID
var EW_PAGE_ID = realmlist_add.PageID; // For backward compatibility

// Form object
var frealmlistadd = new ew_Form("frealmlistadd");

// Validate form
frealmlistadd.Validate = function(fobj) {
...
frealmlistadd.Form_CustomValidate = 
...
frealmlistadd.ValidateRequired = true;
<?php } else { ?>
frealmlistadd.ValidateRequired = false; 
<?php } ?>
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $realmlist->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $realmlist->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $realmlist_add->ShowPageHeader(); ?>
<?php
$realmlist_add->ShowMessage();
?>
<form name="frealmlistadd" id="frealmlistadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="realmlist">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_realmlistadd" class="ewTable">
