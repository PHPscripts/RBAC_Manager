...
$account_banned_view = NULL; // Initialize page object first

class caccount_banned_view extends caccount_banned {
	...
	// Page object name
	var $PageObjName = 'account_banned_view';
	...
	function Page_Main() {
		...
		if ($this->IsPageRequest()) { // Validate request
			if (@$_GET["id"] <> "") {
				...
				$sReturnUrl = "account_bannedlist.php"; // Return to list
			}
			if (@$_GET["bandate"] <> "") {
			...
			switch ($this->CurrentAction) {
				case "I": // Get a record to display
					if (!$this->LoadRow()) { // Load record based on key
						...
						$sReturnUrl = "account_bannedlist.php"; // No matching record, return to list
					}
			}
			...
			$sReturnUrl = "account_bannedlist.php"; // Not page request, return to list
		}
		if ($sReturnUrl <> "")
	...
	function SetupExportOptions() {
		...
		$item->Body = "<a id=\"emf_account_banned\" href=\"javascript:void(0);\" onclick=\"ew_EmailDialogShow({lnk:'emf_account_banned',hdr:ewLanguage.Phrase('ExportToEmail'),key:" . ew_ArrayToJsonAttr($this->RecKey) . ",sel:false});\">" . $Language->Phrase("ExportToEmail") . "</a>";
		$item->Visible = FALSE;
...
// Create page object
if (!isset($account_banned_view)) $account_banned_view = new caccount_banned_view();

// Page init
$account_banned_view->Page_Init();

// Page main
$account_banned_view->Page_Main();
?>
...
// Page object
var account_banned_view = new ew_Page("account_banned_view");
account_banned_view.PageID = "view"; // Page ID
var EW_PAGE_ID = account_banned_view.PageID; // For backward compatibility

// Form object
var faccount_bannedview = new ew_Form("faccount_bannedview");

// Form_CustomValidate event
faccount_bannedview.Form_CustomValidate = 
...
faccount_bannedview.ValidateRequired = true;
<?php } else { ?>
faccount_bannedview.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faccount_bannedview.Lists["x_id"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_id","x_username","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<?php } ?>
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("View") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $account_banned->TableCaption() ?>&nbsp;&nbsp;</span><?php $account_banned_view->ExportOptions->Render("body"); ?>
</p>
<?php if ($account_banned->Export == "") { ?>
<p class="phpmaker">
<a href="<?php echo $account_banned_view->ListUrl ?>" id="a_BackToList" class="ewLink"><?php echo $Language->Phrase("BackToList") ?></a>&nbsp;
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($account_banned_view->AddUrl <> "") { ?>
<a href="<?php echo $account_banned_view->AddUrl ?>" id="a_AddLink" class="ewLink"><?php echo $Language->Phrase("ViewPageAddLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($account_banned_view->EditUrl <> "") { ?>
<a href="<?php echo $account_banned_view->EditUrl ?>" id="a_EditLink" class="ewLink"><?php echo $Language->Phrase("ViewPageEditLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($account_banned_view->CopyUrl <> "") { ?>
<a href="<?php echo $account_banned_view->CopyUrl ?>" id="a_CopyLink" class="ewLink"><?php echo $Language->Phrase("ViewPageCopyLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
<?php if ($Security->IsLoggedIn()) { ?>
<?php if ($account_banned_view->DeleteUrl <> "") { ?>
<a href="<?php echo $account_banned_view->DeleteUrl ?>" id="a_DeleteLink" class="ewLink"><?php echo $Language->Phrase("ViewPageDeleteLink") ?></a>&nbsp;
<?php } ?>
<?php } ?>
</p>
<?php } ?>
<?php $account_banned_view->ShowPageHeader(); ?>
<?php
$account_banned_view->ShowMessage();
?>
<form name="faccount_bannedview" id="faccount_bannedview" class="ewForm" action="" method="post">
<input type="hidden" name="t" value="account_banned">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_account_bannedview" class="ewTable">
<?php if ($account_banned->id->Visible) { // id ?>
	<tr id="r_id"<?php echo $account_banned->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_account_banned_id"><table class="ewTableHeaderBtn"><tr><td><?php echo $account_banned->id->FldCaption() ?></td></tr></table></span></td>
		<td<?php echo $account_banned->id->CellAttributes() ?>><span id="el_account_banned_id">
<span<?php echo $account_banned->id->ViewAttributes() ?>>
<?php echo $account_banned->id->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $account_banned->bandate->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $account_banned->unbandate->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
<?php if ($account_banned->bannedby->Visible) { // bannedby ?>
	<tr id="r_bannedby"<?php echo $account_banned->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_account_banned_bannedby"><table class="ewTableHeaderBtn"><tr><td><?php echo $account_banned->bannedby->FldCaption() ?></td></tr></table></span></td>
...
<?php echo $account_banned->bannedby->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
<?php if ($account_banned->banreason->Visible) { // banreason ?>
	<tr id="r_banreason"<?php echo $account_banned->RowAttributes() ?>>
		<td class="ewTableHeader"><span id="elh_account_banned_banreason"><table class="ewTableHeaderBtn"><tr><td><?php echo $account_banned->banreason->FldCaption() ?></td></tr></table></span></td>
...
<?php echo $account_banned->banreason->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
...
<?php echo $account_banned->active->ViewValue ?></span>
</span></td>
	</tr>
<?php } ?>
</table>
...
<br>
<script type="text/javascript">
faccount_bannedview.Init();
</script>
<?php
$account_banned_view->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<?php if ($account_banned->Export == "") { ?>
...
<?php
$account_banned_view->Page_Terminate();
?>
