...
$autobroadcast_add = NULL; // Initialize page object first

class cautobroadcast_add extends cautobroadcast {
	...
	// Page object name
	var $PageObjName = 'autobroadcast_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "autobroadcastview.php")
	...
	function AddRow($rsold = NULL) {
		...
		// weight
		$this->weight->SetDbValueDef($rsnew, $this->weight->CurrentValue, NULL, strval($this->weight->CurrentValue) == "");
...
// Create page object
if (!isset($autobroadcast_add)) $autobroadcast_add = new cautobroadcast_add();

// Page init
$autobroadcast_add->Page_Init();

// Page main
$autobroadcast_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var autobroadcast_add = new ew_Page("autobroadcast_add");
autobroadcast_add.PageID = "add"; // Page ID
var EW_PAGE_ID = autobroadcast_add.PageID; // For backward compatibility

// Form object
var fautobroadcastadd = new ew_Form("fautobroadcastadd");

// Validate form
fautobroadcastadd.Validate = function(fobj) {
...
fautobroadcastadd.Form_CustomValidate = 
...
fautobroadcastadd.ValidateRequired = true;
<?php } else { ?>
fautobroadcastadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
fautobroadcastadd.Lists["x_realmid"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_id","x_name","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $autobroadcast->TableCaption() ?></span></p>
<p class="phpmaker"><a href="<?php echo $autobroadcast->getReturnUrl() ?>" id="a_GoBack" class="ewLink"><?php echo $Language->Phrase("GoBack") ?></a></p>
<?php $autobroadcast_add->ShowPageHeader(); ?>
<?php
$autobroadcast_add->ShowMessage();
?>
<form name="fautobroadcastadd" id="fautobroadcastadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="autobroadcast">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_autobroadcastadd" class="ewTable">
...
<select id="x_realmid" name="x_realmid"<?php echo $autobroadcast->realmid->EditAttributes() ?>>
<?php
if (is_array($autobroadcast->realmid->EditValue)) {
	$arwrk = $autobroadcast->realmid->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($autobroadcast->realmid->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
<?php if ($arwrk[$rowcntwrk][2] <> "") { ?>
<?php echo ew_ValueSeparator(1,$autobroadcast->realmid) ?><?php echo $arwrk[$rowcntwrk][2] ?>
<?php } ?>
</option>
<?php
	}
...
<script type="text/javascript">
fautobroadcastadd.Lists["x_realmid"].Options = <?php echo (is_array($autobroadcast->realmid->EditValue)) ? ew_ArrayToJson($autobroadcast->realmid->EditValue, 1) : "[]" ?>;
</script>
...
<script type="text/javascript">
fautobroadcastadd.Init();
</script>
<?php
$autobroadcast_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$autobroadcast_add->Page_Terminate();
?>
