...
$account_banned_add = NULL; // Initialize page object first

class caccount_banned_add extends caccount_banned {
	...
	// Page object name
	var $PageObjName = 'account_banned_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "account_bannedview.php")
	...
	function LoadDefaultValues() {
		...
		$this->bandate->CurrentValue = 0;
		$this->unbandate->CurrentValue = 0;
	...
	function AddRow($rsold = NULL) {
		...
		// bandate
		$this->bandate->SetDbValueDef($rsnew, $this->bandate->CurrentValue, 0, strval($this->bandate->CurrentValue) == "");

		// unbandate
		$this->unbandate->SetDbValueDef($rsnew, $this->unbandate->CurrentValue, 0, strval($this->unbandate->CurrentValue) == "");

		// bannedby
		$this->bannedby->SetDbValueDef($rsnew, $this->bannedby->CurrentValue, "", FALSE);

		// banreason
		$this->banreason->SetDbValueDef($rsnew, $this->banreason->CurrentValue, "", FALSE);
...
// Create page object
if (!isset($account_banned_add)) $account_banned_add = new caccount_banned_add();

// Page init
$account_banned_add->Page_Init();

// Page main
$account_banned_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var account_banned_add = new ew_Page("account_banned_add");
account_banned_add.PageID = "add"; // Page ID
var EW_PAGE_ID = account_banned_add.PageID; // For backward compatibility

// Form object
var faccount_bannedadd = new ew_Form("faccount_bannedadd");

// Validate form
faccount_bannedadd.Validate = function(fobj) {
...
faccount_bannedadd.Form_CustomValidate = 
...
faccount_bannedadd.ValidateRequired = true;
<?php } else { ?>
faccount_bannedadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faccount_bannedadd.Lists["x_id"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_id","x_username","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $account_banned->TableCaption() ?></span></p>
...
<?php $account_banned_add->ShowPageHeader(); ?>
<?php
$account_banned_add->ShowMessage();
?>
<form name="faccount_bannedadd" id="faccount_bannedadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="account_banned">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_account_bannedadd" class="ewTable">
...
<select id="x_id" name="x_id"<?php echo $account_banned->id->EditAttributes() ?>>
<?php
if (is_array($account_banned->id->EditValue)) {
	$arwrk = $account_banned->id->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($account_banned->id->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
<?php if ($arwrk[$rowcntwrk][2] <> "") { ?>
<?php echo ew_ValueSeparator(1,$account_banned->id) ?><?php echo $arwrk[$rowcntwrk][2] ?>
<?php } ?>
</option>
<?php
	}
...
<script type="text/javascript">
faccount_bannedadd.Lists["x_id"].Options = <?php echo (is_array($account_banned->id->EditValue)) ? ew_ArrayToJson($account_banned->id->EditValue, 1) : "[]" ?>;
</script>
...
<input type="text" name="x_bandate" id="x_bandate" size="30" value="<?php echo $account_banned->bandate->EditValue ?>"<?php echo $account_banned->bandate->EditAttributes() ?>>
...
<script type="text/javascript">
faccount_bannedadd.Init();
</script>
<?php
$account_banned_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$account_banned_add->Page_Terminate();
?>
