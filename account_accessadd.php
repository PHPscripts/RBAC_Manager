...
$account_access_add = NULL; // Initialize page object first

class caccount_access_add extends caccount_access {
	...
	// Page object name
	var $PageObjName = 'account_access_add';
		...
		switch ($this->CurrentAction) {
			...
			case "A": // Add new record
				...
				if ($this->AddRow($this->OldRecordset)) { // Add successful
					...
					if (ew_GetPageName($sReturnUrl) == "account_accessview.php")
	...
	function LoadDefaultValues() {
		...
		$this->gmlevel->CurrentValue = NULL;
		$this->gmlevel->OldValue = $this->gmlevel->CurrentValue;
	...
	function AddRow($rsold = NULL) {
		...
		// gmlevel
		$this->gmlevel->SetDbValueDef($rsnew, $this->gmlevel->CurrentValue, 0, FALSE);
...
// Create page object
if (!isset($account_access_add)) $account_access_add = new caccount_access_add();

// Page init
$account_access_add->Page_Init();

// Page main
$account_access_add->Page_Main();
?>
<?php include_once "header.php" ?>
<script type="text/javascript">

// Page object
var account_access_add = new ew_Page("account_access_add");
account_access_add.PageID = "add"; // Page ID
var EW_PAGE_ID = account_access_add.PageID; // For backward compatibility

// Form object
var faccount_accessadd = new ew_Form("faccount_accessadd");

// Validate form
faccount_accessadd.Validate = function(fobj) {
...
faccount_accessadd.Form_CustomValidate = 
...
faccount_accessadd.ValidateRequired = true;
<?php } else { ?>
faccount_accessadd.ValidateRequired = false; 
<?php } ?>

// Dynamic selection lists
faccount_accessadd.Lists["x_id"] = {"LinkField":"x_id","Ajax":null,"AutoFill":false,"DisplayFields":["x_id","x_username","",""],"ParentFields":[],"FilterFields":[],"Options":[]};
...
<p><span id="ewPageCaption" class="ewTitle ewTableTitle"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $account_access->TableCaption() ?></span></p>
...
<?php $account_access_add->ShowPageHeader(); ?>
<?php
$account_access_add->ShowMessage();
?>
<form name="faccount_accessadd" id="faccount_accessadd" class="ewForm" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return ewForms[this.id].Submit();">
<br>
<input type="hidden" name="t" value="account_access">
<input type="hidden" name="a_add" id="a_add" value="A">
<table cellspacing="0" class="ewGrid"><tr><td class="ewGridContent">
<div class="ewGridMiddlePanel">
<table id="tbl_account_accessadd" class="ewTable">
...
<select id="x_id" name="x_id"<?php echo $account_access->id->EditAttributes() ?>>
<?php
if (is_array($account_access->id->EditValue)) {
	$arwrk = $account_access->id->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($account_access->id->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
<?php if ($arwrk[$rowcntwrk][2] <> "") { ?>
<?php echo ew_ValueSeparator(1,$account_access->id) ?><?php echo $arwrk[$rowcntwrk][2] ?>
<?php } ?>
</option>
<?php
	}
...
<script type="text/javascript">
faccount_accessadd.Lists["x_id"].Options = <?php echo (is_array($account_access->id->EditValue)) ? ew_ArrayToJson($account_access->id->EditValue, 1) : "[]" ?>;
</script>
...
<select id="x_RealmID" name="x_RealmID"<?php echo $account_access->RealmID->EditAttributes() ?>>
<?php
if (is_array($account_access->RealmID->EditValue)) {
	$arwrk = $account_access->RealmID->EditValue;
	$rowswrk = count($arwrk);
	$emptywrk = TRUE;
	for ($rowcntwrk = 0; $rowcntwrk < $rowswrk; $rowcntwrk++) {
		$selwrk = (strval($account_access->RealmID->CurrentValue) == strval($arwrk[$rowcntwrk][0])) ? " selected=\"selected\"" : "";
		if ($selwrk <> "") $emptywrk = FALSE;
?>
<option value="<?php echo ew_HtmlEncode($arwrk[$rowcntwrk][0]) ?>"<?php echo $selwrk ?>>
<?php echo $arwrk[$rowcntwrk][1] ?>
...
<script type="text/javascript">
faccount_accessadd.Init();
</script>
<?php
$account_access_add->ShowPageFooter();
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
?>
<script type="text/javascript">
...
<?php
$account_access_add->Page_Terminate();
?>
